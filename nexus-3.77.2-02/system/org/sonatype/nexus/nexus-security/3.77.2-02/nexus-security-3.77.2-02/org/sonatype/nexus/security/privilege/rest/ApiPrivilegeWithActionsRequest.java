...
extends ApiPrivilegeRequest {
    ...
    public ApiPrivilegeWithActionsRequest(String name, String description, Collection<PrivilegeAction> actions) {
    ...
    protected String toCrudTaskActionString() {
        ...
        List actionList = this.actions.stream().map(PrivilegeAction::getCrudTaskActions).filter(Objects::nonNull).collect(Collectors.toList());
        return String.join((CharSequence)",", actionList);
    }
